...
public class SalaryController {
    ...
    public Result<PageVO<HrSalaryArchiveVO>> pageArchive(
        ...
        return Result.success(salaryService.pageArchive(pageNum, pageSize, employeeId, keyword, gradeCode));
    }
    ...
    @Operation(summary = "提交薪资档案变更去审批")
    @PreAuthorize("hasPermission(T(com.gbi.platform.common.constant.PermissionConst).HR_SALARY_ARCHIVE_EDIT,'')")
    @PostMapping("/archive/submitAudit/{id}")
    public Result<Void> submitArchiveAudit(@PathVariable Long id) {
        salaryService.submitArchiveAudit(id);
        return Result.success();
    }
