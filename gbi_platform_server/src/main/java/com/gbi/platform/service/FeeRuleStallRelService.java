...
/**
 * 收费规则-摊位绑定服务（biz_fee_rule_stall_rel）
 * 摊位多选收费规则，同一收费类型（biz_fee_item）限选一条
 *
 * @author gbi
 */
public interface FeeRuleStallRelService {
    ...
    /**
     * 查询摊位已绑定收费规则（回显用，company_id 自动隔离）
     *
     * @param stallId 摊位ID
    ...
    /**
     * 批量查询多个摊位已绑定收费规则（分页列表组装用，一次查询避免 N+1，company_id 自动隔离）
     *
     * @param stallIds 摊位ID集合
     * @return stallId → 已绑定规则列表（无绑定的摊位不包含在 map 中）
     */
     ...
     * </ul>
     * 校验规则：摊位必须存在（company 自动隔离）；每条规则必须存在且启用（停用/跨公司拦截）；
